...
dfm_amazon_train_pros<-MLTASS_dfm(gd_amazon_train$pros,ngrams=1:2) %>%
  convert(to="matrix")
...
dfm_amazon_test_pros<-MLTASS_dfm(gd_amazon_test$pros,
                              ngrams=1:2,
                              min.prop = 0) %>%
...
dfm_amazon_test_cons<-MLTASS_dfm(gd_amazon_test$cons,ngrams=1:2)  %>%
  dfm_match(colnames(dfm_amazon_train_pros)) %>%
  convert(to="matrix")
...
dfm_amazon_train_cons<-MLTASS_dfm(gd_amazon_train$cons,ngrams=1:2) %>%
  convert(to="matrix")
...
dfm_amazon_test_pros<-MLTASS_dfm(gd_amazon_test$pros,ngrams=1:2)  %>%
  dfm_match(colnames(dfm_amazon_train_cons)) %>%
  convert(to="matrix")
...
################
################
dfm_amazon_train_pros<-MLTASS_dfm(gd_amazon_train$pros,
                               ngrams=2:3) %>%
  convert(to="matrix")
...
dfm_amazon_train_cons<-MLTASS_dfm(gd_amazon_train$cons,
                               ngrams=2:3) %>%
  convert(to="matrix")
...
dfm_amazon_test_cons<-MLTASS_dfm(gd_amazon_test$cons,
                              ngrams=2:3, 
                              min.prop = 0)  %>%
  dfm_match(colnames(dfm_amazon_train_cons)) %>%
  convert(to="matrix")
...
# create our prediction variables from the cons text
dfm_microsoft_train_cons<-MLTASS_dfm(gd_microsoft_train$cons,ngrams=1:2) %>%
  convert(to="matrix")

microsoft_model_cons<-cv.glmnet(x=dfm_microsoft_train_cons,
                                y=microsoft_train_Y)

dfm_microsoft_test_cons<-MLTASS_dfm(gd_microsoft_test$cons,ngrams=1:2)  %>%
