...
kendall_acc(rev_med_train$BLsentiment,
            rev_med_train$stars)
...
kendall_acc(rev_med_train$VDsentiment,
            rev_med_train$stars)
...
train_cats<-MLTASS_dfm(rev_med_train$categories)%>%
  convert(to="data.frame") %>%
  as_tibble() %>%
...
test_cats<-MLTASS_dfm(rev_med_test$categories)%>%
  convert(to="data.frame") %>%
  as_tibble() %>%
  select(chines,sandwich,nightlif,mexican) 
...
one_cat_dfm_train<-MLTASS_dfm(one_cat_train$text,ngrams=1)

one_cat_dfm_test<-MLTASS_dfm(one_cat_test$text,
                          ngrams=1,
                          min.prop=0) %>%
