...
MLTASS_dfm(dox, ngrams=2)

MLTASS_dfm(dox, stop.words = FALSE)

MLTASS_dfm(dox, min.prop=.25)
...
dfm1<-MLTASS_dfm(review_dat$text,
              ngrams=1,
              min.prop=0,
              stop.words = FALSE)
...
dfm3<-MLTASS_dfm(review_dat$text,ngrams=1) %>%
  convert(to="data.frame") %>%
  select(-doc_id)
